...
RESPONSE="$(curl -kLsS https://api.mullvad.net/public/relays/wireguard/v1/)" || die "Unable to connect to Mullvad API."
...
for CODE in "${SERVER_CODES[@]}"; do
	CONFIGURATION_FILE="$PWD/configs/$CODE.conf"
...
RESPONSE="$(curl -skSL https://api.mullvad.net/wg/ -d account="$ACCOUNT" --data-urlencode pubkey="$(wg pubkey <<<"$PRIVATE_KEY")")" || die "Could not talk to Mullvad API."
[[ $RESPONSE =~ ^[0-9a-f:/.,]+$ ]] || die "$RESPONSE"
ADDRESS=$(echo $RESPONSE| sed -e 's/,/ #/gi')
DNS="193.138.219.228"
...
for CODE in "${SERVER_CODES[@]}"; do
	...
	cat > "$CONFIGURATION_FILE.tmp" <<-_EOF
		...
		Address = $ADDRESS
		DNS = 193.138.218.74,1.1.1.1,8.8.8.8
		[Peer]
...
echo "[+] Success. We have the following locations:"
for CODE in "${SERVER_CODES[@]}"; do
